...
namespace Hypernex.Game
{
    public partial class AvatarRoot : Node
    {
        ...
        public async void InitIk()
        {
            ...
            ikSystem.head = "Head";
            ikSystem.hips = "Hips";
            ikSystem.leftHand = "LeftHand";
            ikSystem.rightHand = "RightHand";
            ikSystem.leftUpperLeg = "LeftUpperLeg";
            ikSystem.rightUpperLeg = "RightUpperLeg";
            ikSystem.leftFoot = "LeftFoot";
            ikSystem.rightFoot = "RightFoot";
        ...
        public void ProcessIkFBT(bool useHead, Transform3D head, Transform3D floor, Transform3D leftHand, Transform3D rightHand, Transform3D hips, Transform3D leftFoot, Transform3D rightFoot)
        {
            if (IsInstanceValid(ikSystem) && ikSystem.IsNodeReady())
            {
                ...
                Vector3 scl = ikSystem.GetBoneTransform(ikSystem.hips).Basis.Scale;
        ...
        public void ProcessIk(bool vr, bool useHead, Transform3D head, Transform3D floor, Transform3D leftHand, Transform3D rightHand)
        {
            if (IsInstanceValid(ikSystem) && ikSystem.IsNodeReady())
            {
                floor = floor * new Transform3D(ikSystem.humanBindPose.Basis.Inverse(), Vector3.Zero);
                // head = head * ikSystem.humanoid.GetBoneGlobalRest(ikSystem.head.BoneIdx).AffineInverse();
                // leftHand = leftHand * ikSystem.GetRestPose(ikSystem.leftHand).AffineInverse();
                // rightHand = rightHand * ikSystem.GetRestPose(ikSystem.rightHand).AffineInverse();
                Node3D root = ikSystem.humanoid;
