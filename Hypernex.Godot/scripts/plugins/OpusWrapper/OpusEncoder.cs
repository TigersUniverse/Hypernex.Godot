...
namespace FragLabs.Audio.Codecs
{
    ...
    public class OpusEncoder : IDisposable
    {
        ...
        public unsafe byte[] EncodeFloat(float[] inputPcmSamples, int sampleLength, out int encodedLength)
        {
            if (disposed)
                throw new ObjectDisposedException("OpusEncoder");
            ...
            int frames = FrameCount(inputPcmSamples);
            IntPtr encodedPtr;
            byte[] encoded = new byte[sampleLength * sizeof(float)];
            int length = 0;
            fixed (byte* benc = encoded)
            {
                encodedPtr = new IntPtr((void*)benc);
                length = API.opus_encode_float(_encoder, inputPcmSamples, inputPcmSamples.Length / InputChannels, encodedPtr, encoded.Length);
            }
            encodedLength = length;
            if (length < 0)
                throw new Exception("Encoding failed - " + ((Errors)length).ToString());
            // encodedLength *= sizeof(float);
        ...
        public int FrameCount(float[] pcmSamples)
        {
            ...
            // return (int)((float)pcmSamples.Length / InputSamplingRate);
            int bitrate = 16;
            int bytesPerSample = (bitrate / 8) * InputChannels;
            return pcmSamples.Length /** sizeof(float)*/ / bytesPerSample;
        }
